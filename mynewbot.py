...
log = logging.getLogger("NICE-BOT")

user_state = {}

# ============================================
#   KEYBOARD MAKER (REVERSED ORDER - NICE DESIGN)
# ============================================
def make_keyboard(options):
    options = [opt[0] if isinstance(opt, tuple) else opt for opt in options]
    options.reverse()   # عكس الترتيب علشان اليمين يبقى شمال
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        "👑✨ *أهلاً بكم في منصّة نيو أكاديمي* ✨👑\n"
        "-----------------------------------------\n"
        "📚 أقوى منصة تعليمية للطلاب\n"
        "📝 مذكرات – 📘 اختبارات – 🎥 فيديوهات شرح\n"
        "-----------------------------------------\n"
        "💡 *اختر المرحلة للمتابعة:*"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["step"] == "subchild":
        ...
        """, (state["subject_id"], state["option_id"], state["child_id"], sub_id))
